	...
	<div id="wrapper">
											...
											<c:forEach items="${menu.nodes}" var="item" varStatus="status">
												<ul class="nav nav-second-level">
													<shiro:hasPermission name="${item.permission}">
														<li><a class="J_menuItem" href="${ctx}/${!empty item.url?item.url:''}">${item.name}</a> <c:if test="${!empty item.nodes&&fn:length(item.nodes)!=0}">
																<ul class="nav nav-third-level">
																	<c:if test="${!empty item.nodes&&fn:length(item.nodes)!=0}">
																		<c:forEach items="${item.nodes}" var="node" varStatus="index">
																			<c:if test="${node.type.code!='res-op'}">
		...
		<div id="page-wrapper" class="gray-bg dashbard-1">
			...
			<div class="row J_mainContent" id="content-main">
				<iframe class="J_iframe" name="iframe0" width="100%" height="100%" src="<%=basePath%>/sys/main" frameborder="0" data-id="<%=basePath%>/sys/main" seamless></iframe>
			</div>
			<div class="footer">
