...
<%-- 				<span id="jump_multi"> <a href="#" class="btn btn-primary op_btn" data-options="iconCls:'icon-logout'" onclick="javascript:jump(2,'${taskId}')">越级审批</a> --%>
			...
			<a href="${webPath}/process/task/viewPage/${processModel.proInstanceId}?type=image" class="btn btn-warning op_btn right" data-options="iconCls:'icon-layout-content'" target="blank">查看流程图</a>
			<c:if test="${(sourcePage==1) }">
<%-- 				<a style="margin-left: 50px" href="#" class="btn btn-danger op_btn right" data-options="iconCls:'icon-stop'" onclick="javascript:endProcess('${taskId}','${_userId}')">结束流程</a> --%>
			</c:if>
		</div>
	</div>
	<!-- 批注 -->
	...
	<c:if test="${!empty taskId||!empty processInstanceId}">
		...
		<div class="row">
			<div class="wlf panel panel-default col-8 col-sm-8" style="margin-left: 15px; padding-left: 0px; padding-right: 0px; font-size:14px;width:996px;">
