...
function initSelect() {
	...
	if (option != undefined && option != "") {
		...
		success : function(result) {
		    ...
		    for (var i = 0; i < result.length; i++) {
			var obj = result[i];
			html += '<option value="' + obj[_value] + '"' + ' data-rowdata=\'' + obj + '\'';
			if (_thisvalue != "") {
			    if (_thisvalue == obj[_value]) {
				html += ' selected="selected"';
			    }
	...
	if (option != undefined && option != "") {
		...
		success : function(result) {
			...
			html += '<input type="radio" name="' + _name + '" value="' + obj[_value] + '"';
			// if (i == 0 && _thisvalue == "") {
			// html += ' checked="true"';
			// } else
			if (_thisvalue == obj[_value]) {
	...
	if (option != undefined && option != "") {
		...
		success : function(result) {
			...
			var obj = result[i];
			html += '<label class="customLabel">\n';
			html += '<input type="checkbox" name="' + _name + '" value="' + obj[_value] + '"';
			// if (i == 0 && _thisvalue == "") {
			// html += ' checked="true"';
			// } else
			if (_thisvalue == obj[_value]) {
