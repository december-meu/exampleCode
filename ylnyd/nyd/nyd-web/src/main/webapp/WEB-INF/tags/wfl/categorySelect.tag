...
<c:choose>
	<c:when test="${!empty wrapped&&wrapped=='false'}">
		<div class="${empty contentClass?'col-sm-10 col-xs-10':contentClass}">
			<input id="category-tree-Select" type="text" class="form-control dd-dropdown-tree" readonly="readonly" style="background:#fff" data-tree-href="${ctx}/process/category/tree" data-tree-view="_category-tree"
				id="input-categoryartment" placeholder="流程类别"> <input type="hidden" name="category" id="category-tree-Select-id" value="${categoryId}">
		</div>
		<div id="_category-tree" style="display: none;"></div>
	</c:when>
	<c:otherwise>
		<div class="form-group">
			<c:if test="${empty disableLabel||disableLabel!=true}">
				<label for="input-categoryartment" class="${empty labelClass?'col-sm-2 col-xs-2 text-right':labelClass}">类别:</label>
			</c:if>
			<div class="${empty contentClass?'col-sm-10 col-xs-10':contentClass}">
				<input id="category-tree-Select" type="text" class="form-control dd-dropdown-tree" readonly="readonly" data-tree-href="${ctx}/process/category/tree" data-tree-view="_category-tree"
					id="input-categoryartment" placeholder="流程类别"> <input type="hidden" name="category" id="category-tree-Select-id" value="${categoryId}">
			</div>
			<div id="_category-tree" style="display: none;"></div>
		</div>
<!-- 		<table id="_categroy_tree"></table> -->
	</c:otherwise>
...
<script type="text/javascript">
	...
	$(document).click( function() {
	    var viewId = $('#category-tree-Select').attr("data-tree-view");
	    if ($('#category-tree-Select').hasClass("show-view")) {
		$("#" + viewId).hide();
		$('#category-tree-Select').removeClass("show-view");
	    }
	});
    }
	...
	var posting = $.post(href, {}, function(resp) {
	    ...
	    _initTreeView(dom, resp);
	});
    }
    function _initTreeView(dom, data) {
