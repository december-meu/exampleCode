...
(function($) {
	...
	for ( var p in props) {
	    if (value ==undefined || !value.hasOwnProperty(props[p])) {
		value = "";
		return;
	    } else {
		value = value[props[p]];
	...
	var items = $.grep(source, function(item, index) {
	    return node[that.options.parentId] == item[field];
	});
    ...
    var getChild = function(node, source, field, parentField) {
	var items = $.grep(source, function(item, index) {
	    return item[parentField] == node[field];
	});
	return items;
    };

    var getAllChild = function(node, source, field, parentField) {
	var data = [];
	var g = function(child) {
	    $.each(child, function(i, n) {
		data.push(n);
		var subChild = getChild(n, source, field, parentField);
		if (subChild != null && subChild.length > 0) {
		    g(subChild);
		}
	    });
	}
	var child = getChild(node, source, field, parentField);
	g(child);
	return data;
    };

    // 调用bootstrapTable组件的构造器得到对象
    var BootstrapTable = $.fn.bootstrapTable.Constructor, _initData = BootstrapTable.prototype.initData, _initPagination = BootstrapTable.prototype.initPagination;
	...
	html.push('<tr', sprintf(' %s', htmlAttributes.join(' ')), sprintf(' id="%s"', $.isArray(item) ? undefined : item._id), sprintf(' class="%s"',
	    style.classes || ($.isArray(item) ? undefined : item._class)), sprintf(' data-index="%s"', i), sprintf(' data-uniqueid="%s"',
	...
	if (!this.options.cardView && this.options.detailView) {
	    html.push('<td>', '<a class="detail-icon" href="#">', sprintf('<i class="%s %s"></i>', this.options.iconsPrefix, this.options.icons.detailOpen),
		'</a>', '</td>');
	}
		    ...
		    if (column.checkbox || column.radio) {
		        ...
		        text = [
		            ...
		            '<input'
		                ...
		                + sprintf(' checked="%s"', value === true || (value_ || value && value.checked) || (item && item.checked) ? 'checked'
		                    : undefined) + sprintf(' disabled="%s"', !column.checkboxEnabled || (value && value.disabled) ? 'disabled' : undefined)
		                + ' />', that.header.formatters[j] && typeof value === 'string' ? value : '', that.options.cardView ? '</div>' : '</td>' ]
		            .join('');
			    ...
			    var indent = item.level == that.options.treeRootLevel ? '' : sprintf('<span style="margin-left: %spx;"></span>',
			    ...
			    icon = sprintf('<span class="tree-icon %s" style="cursor: pointer; margin: 0px 5px;"></span>',
		        ...
		        text = that.options.cardView ? [
		            '<div class="card-view">',
		            that.options.showHeader ? sprintf('<span class="title" %s>%s</span>', style, getPropertyFromOther(that.columns, 'field', 'title',
		                field)) : '', sprintf('<span class="value">%s</span>', value), '</div>' ].join('') : [
		            sprintf('<td%s %s %s %s %s %s %s>', id_, class_, style, data_, rowspan_, colspan_, title_), indent, icon, value, '</td>' ].join('');
	...
	this.$body
	    ...
	    .on(
	        'click dblclick',
	        function(e) {
		    var $td = $(this), $tr = $td.parent(), item = that.data[$tr.data('index')], index = $td[0].cellIndex, fields = that.getVisibleFields(), field = fields[that.options.detailView
		        && !that.options.cardView ? index - 1 : index], column = that.columns[getFieldIndex(that.columns, field)], value = getItemField(item,
	...
	this.$selectItem.off('click').on('click', function(event) {
	    ...
	    var child = getAllChild(row, that.options.data, that.options.treeId, that.options.parentId);
	    $.each(child, function(i, c) {
		$.each(that.data, function(index, item) {
		    if (item[that.options.treeId] == c[that.options.treeId]) {
